...
function serc_frontend_assets()
{
    ...
    wp_enqueue_script('serc-frontend', plugins_url('assets/js/serc-frontend.js', __FILE__), array('jquery', 'jquery-mask'), '1.33', true);
    ...
    wp_enqueue_style('serc-dashboard-style', plugins_url('assets/css/style.css', __FILE__), array(), '1.33');
...
function serc_cnpj_form_shortcode($atts)
{
    ...
    <form id="serc-cnpj-form">
        ...
        <button type="submit"
            style="background-color:#009C3B;color:#fff;border:NONE;border-radius:5px;margin-top:10px;">Consultar</button>
...
function serc_render_form($type, $fields)
{
    ...
    <form class="serc-form" data-type="<?php echo esc_attr($type); ?>">
        <?php foreach ($fields as $f): ?>
            ...
            <input id="<?php echo esc_attr($f['name']); ?>" name="<?php echo esc_attr($f['name']); ?>" <?php if (!empty($f['class']))
                      echo 'class="' . esc_attr($f['class']) . '"'; ?>
                type="<?php echo esc_attr($f['type'] ?? 'text'); ?>"
                placeholder="<?php echo esc_attr($f['placeholder'] ?? ''); ?>" <?php if (!empty($f['pattern']))
                         echo 'pattern="' . esc_attr($f['pattern']) . '"'; ?>         <?php if (!empty($f['title']))
                                            echo 'title="' . esc_attr($f['title']) . '"'; ?>         <?php if (!empty($f['inputmode']))
                                                               echo 'inputmode="' . esc_attr($f['inputmode']) . '"'; ?>         <?php if (!empty($f['required']))
                                                                                  echo 'required'; ?> />
        <?php endforeach; ?>
